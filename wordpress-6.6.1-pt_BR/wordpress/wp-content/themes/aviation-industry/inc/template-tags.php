...
if( !function_exists('aviation_industry_site_logo') ):
     ...
     */
    function aviation_industry_site_logo( $args = array(), $echo = true ){
         ...
         */
        $args = apply_filters('aviation_industry_site_logo_args', $args, $defaults);
        if ( has_custom_logo() ) {
         ...
         */
        $html = apply_filters('aviation_industry_site_logo', $html, $args, $classname, $contents);
        if (!$echo) {
            return $html;
        }
...
if( !function_exists('aviation_industry_site_description') ):
     ...
     */
    function aviation_industry_site_description($echo = true){
         ...
         *
         */
        $html = apply_filters('aviation_industry_site_description', $html, $description, $wrapper);
        if (!$echo) {
            return $html;
        }
        echo $html; //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
...
if( !function_exists('aviation_industry_posted_on') ):
     ...
     */
    function aviation_industry_posted_on( $icon = true, $animation_class = '' ){
        ...
        if( $aviation_industry_post_date ){
            ...
            if( $icon ){
                ...
                aviation_industry_the_theme_svg('calendar');
                echo '</span>';
...
if( !function_exists('aviation_industry_posted_by') ) :
     ...
     */
    function aviation_industry_posted_by( $icon = true, $animation_class = '' ){   
        ...
        if( $aviation_industry_post_author ){
            ...
            if( $icon ){
                ...
                aviation_industry_the_theme_svg('user');
                echo '</span>';
...
if( !function_exists('aviation_industry_posted_by_avatar') ) :
     ...
     * Prints HTML with meta information for the current author.
     */
    function aviation_industry_posted_by_avatar( $date = false ){
        ...
        if( $aviation_industry_post_author ){
            ...
            if( $date ){
                aviation_industry_posted_on($icon = false);
            }
            echo '</div>';
...
if( !function_exists('aviation_industry_entry_footer') ):
     ...
     */
    function aviation_industry_entry_footer( $cats = true, $tags = true, $edits = true){   
        ...
        if ('post' === get_post_type()) {
            ...
            if( $cats && $aviation_industry_post_category ){
            ...
            if( $tags && $aviation_industry_post_tags ){
                /* translators: used between list items, there is a space after the comma */
                $tags_list = get_the_tag_list('', esc_html_x(', ', 'list item separator', 'aviation-industry'));
                if( $tags_list ){
                ...
                edit_post_link(
                    sprintf(
                        ...
                        /* translators: %s: Name of current post. Only visible to screen readers */
                            __('Edit <span class="screen-reader-text">%s</span>', 'aviation-industry'),
                            array(
                                'span' => array(
                                    'class' => array(),
                                ),
...
if ( !function_exists('aviation_industry_post_thumbnail') ) :
     ...
     */
    function aviation_industry_post_thumbnail($image_size = 'full'){
...
if( !function_exists('aviation_industry_is_comment_by_post_author') ):
     ...
     * @return bool
     */
    function aviation_industry_is_comment_by_post_author($comment = null){
...
if( !function_exists('aviation_industry_breadcrumb') ) :

    /**
     * Aviation Industry Breadcrumb
     */
    function aviation_industry_breadcrumb($comment = null){
